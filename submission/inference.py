...
def interf1_handler():
    ...
    # NOTE: CUDA info is logged from inside the spawned child (src/interf1/model.py), not here.
    # Calling show_torch_cuda_info() in this parent process would import torch and initialize a
    # CUDA context that's immediately discarded once we spawn (not fork) the child -- doubling
    # CUDA init overhead per case for a print statement, and eating into the hard timeout budget
    # for no benefit.
