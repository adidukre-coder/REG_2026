"""
Free, read-only check: does the forgotten reg_attr_v2_best.pt (per_task_attention=True,
epoch=4, trained Jun 27 on the SAME split as the real deployed baseline) actually beat the
deployed reg_attr_analysis.pt (shared attention) under TODAY's corrected compose_report.py
and the real production threshold (0.25)?

The numbers stored inside reg_attr_v2_best.pt were measured against whatever compose_report.py
existed on Jun 27 (pre-dating this project's composer/scoring audit fixes) and only on a fast
~50/organ val_eval subset, not the full val set. This script re-evaluates BOTH checkpoints,
from scratch, on the FULL val set, with today's compose_report.py, so the comparison is honest
and apples-to-apples. Loads checkpoints read-only; writes nothing back to any checkpoint or
...
PER_ORGAN = 1600  # matches train_v2.py's split exactly
...
v2_ckpt = "/nfs-stor/adinath.dukre/adinath/reg2026_work/checkpoints/reg_attr_v2_best.pt"

a_acc, a_edge, a_rep = evaluate_full(base_ckpt, "DEPLOYED BASELINE (shared attention)")
b_acc, b_edge, b_rep = evaluate_full(v2_ckpt, "reg_attr_v2_best.pt (per_task_attention, epoch 4, Jun 27)")

print("\n=== DELTA (v2 minus deployed baseline), today's corrected compose_report.py, threshold=0.25 ===")
