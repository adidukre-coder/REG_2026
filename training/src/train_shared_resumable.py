"""
Resumable variant of analyze_performance_seeded.py (per_task_attention=False, shared
attention -- the control arm of the full-corpus per-task-attention A/B). Identical recipe
and split logic to analyze_performance_seeded.py; the only addition is a checkpoint-per-
epoch + resume-on-restart so a long full-corpus run can survive being killed (walltime,
preemption, node failure, session end) without losing completed epochs. Not used to modify
analyze_performance_seeded.py itself since other historical experiments reference it as-is.

Usage: REG_SEED=42 REG_FEAT_DIR=... REG_OUT_CKPT=... python src/train_shared_resumable.py <per_organ> <epochs>
"""
...
print(f"SEED={SEED}  per_task_attention=False  LAB={LAB}  LM={LM}", flush=True)
...
out_ckpt = os.environ.get("REG_OUT_CKPT", f"{ck_dir}/reg_attr_shared_seed{SEED}.pt")
...
for ep in range(start_ep, EPOCHS + 1):
    ...
    torch.save({"model": model.state_dict(), "opt": opt.state_dict(), "label_maps": lm,
                "per_task_attention": False, "epoch": ep, "done": False}, tmp)
    os.replace(tmp, out_ckpt)
    print(f"  checkpoint saved (epoch {ep}) -> {out_ckpt}", flush=True)

# ===== EVAL (only reached once all epochs done -- also runs if resumed exactly at EPOCHS) =====
...
tmp = out_ckpt + ".tmp"
torch.save({"model": model.state_dict(), "opt": opt.state_dict(), "label_maps": lm,
            "per_task_attention": False, "epoch": EPOCHS, "done": True}, tmp)
os.replace(tmp, out_ckpt)
