...
SEG_WORKERS = int(os.environ.get("REG_SEG_WORKERS", "8"))  # parallel seg tile loading
SEG_BATCH = 64

MPP = 0.5            # challenge TIFFs report bogus mpp -> force 20x = 0.5 um/px
MAG = 20
PATCH = 256
SEGMENTER = os.environ.get("REG_SEGMENTER", "hest")   # 'hest' (training) or 'otsu' (fast)
ENCODER = os.environ.get("REG_ENCODER", "hoptimus0")  # 'hoptimus0' (training/deployed) or 'hoptimus1' (pilot)
MAX_PATCHES = int(os.environ.get("REG_MAX_PATCHES", "2000"))   # cap so feature extraction finishes under the 5-min/case T4 limit (env-configurable for diagnostics; submitted container uses 1000)
BATCH_LIMIT = 32
STAIN_NORM = os.environ.get("REG_STAIN_NORM", "none")   # 'none' (training/deployed) or 'macenko' (pilot)


class _MacenkoTransform:
    """Prepends Macenko stain normalization to a patch encoder's eval_transforms.
    Falls back to the un-normalized patch on failure -- near-white/background patches
    have no discernible stain signal, which makes the underlying least-squares stain
    separation singular/degenerate on a small fraction of patches."""
    ...
    def __call__(self, pil_img):
        arr = np.array(pil_img.convert("RGB"))  # copy (not asarray) -- must be writable for torch.from_numpy
        t = torch.from_numpy(arr).permute(2, 0, 1)
        try:
            norm, _, _ = self.normalizer.normalize(I=t, stains=False)
            pil_img = Image.fromarray(norm.numpy().astype(np.uint8))
        except Exception:
            self.n_fail += 1
        return self.base(pil_img)
...
def slide_to_features(wsi_path: str, gpu: int = 0, work_dir: str | None = None) -> torch.Tensor:
    ...
    with load_wsi(slide_path=path, lazy_init=False, mpp=MPP) as slide:
        ...
        if STAIN_NORM == "macenko":
            enc.eval_transforms = _MacenkoTransform(enc.eval_transforms)
    ...
    if STAIN_NORM == "macenko" and isinstance(enc.eval_transforms, _MacenkoTransform):
        print(f"[interf1] macenko fallback (un-normalized) on {enc.eval_transforms.n_fail} patches", flush=True)
    return feats
